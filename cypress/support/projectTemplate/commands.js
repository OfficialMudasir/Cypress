...
Cypress.Commands.add("createPT", (ptName) => {
  ...
  //step2
  cy.get(
    "#kt_wizard_form_step_2_form > .kt-form__section > :nth-child(2) > div > .form-control"
  ).type(stepname1);
  cy.get(".modal-title > span").click();
  //
  cy.get(
    "#kt_wizard_form_step_2_form > .kt-form__section > :nth-child(3) > .form-control"
  ).select("616f955e-05e1-4f4e-b94e-01767440c070"); //select feedback, hard code;//select feedback, hard code, f7e5342f-55c7-4c21-bae9-78f0e4665e41
  cy.get(
    "#kt_wizard_form_step_2_form > .kt-form__section > :nth-child(4) > .form-control"
  ).type(stepdescp1);
  cy.get("#btn-add-ProjectStep").click();

  cy.get(":nth-child(2) > :nth-child(2) > div > .form-control").type(stepname2);
  cy.get(".modal-title > span").click();
  cy.get(":nth-child(2) > :nth-child(3) > .form-control").select(
    "616f955e-05e1-4f4e-b94e-01767440c070"
  ); //select feedback, hard code;//select feedback, hard code
...
Cypress.Commands.add("ptView", (ptName) => {
  cy.get(NameInTableSelector).each(($e, index) => {
    ...
    if (formN.includes(ptName)) {
      ...
      cy.get(".modal-body >.form-group").should(
        "contain",
        "Description of " + ptName
      );
